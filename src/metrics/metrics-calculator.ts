...
interface UiMessage {
  ...
  text?: string;
  ts?: number;
  conversationHistoryIndex?: number | string;
}
...
class MetricsCalculator {
  ...
  async calculate(): Promise<TaskMetrics> {
    ...
    try {
      ...
      const [apiCallCount, userMessageCount, tokenMetrics, model] =
        await Promise.all([
      ...
      const {
        ...
        cacheReads = 0,
        conversationHistoryIndex = 0,
      } = tokenMetrics ?? DEFAULT_METRICS;

      // Validate and calculate duration
      let duration = (this.segment.endTime as number) - this.segment.startTime;

      // Check for unreasonable duration (more than 24 hours)
      const MAX_DURATION = 24 * 60 * 60 * 1000; // 24 hours in milliseconds
      if (duration < 0 || duration > MAX_DURATION) {
        logger.warn(
          `Invalid duration detected for task ${this.segment.taskNumber}: ${duration}ms. Capping at 24 hours.`,
        );
      ...
      return MetricsCalculator.createDefaultResult(
        this.segment,
        this.directoryId,
      );
    }
   ...
   */
  async calculateApiCalls(): Promise<number> {
    ...
    logger.info(
      `API Call Count for Task ${this.segment.taskNumber}: ${apiCallCount}`,
    );
   ...
   */
  async calculateUserMessages(): Promise<number> {
    ...
    this.segment.apiCalls.forEach((entry) => {
      ...
      if (
        ...
        logger.info(
          `Found initial task instruction for task ${this.segment.taskNumber}`,
        );
      ...
      if (
        ...
        logger.info(
          `Found follow-up user message for task ${this.segment.taskNumber}: "${fullText.substring(0, 50)}..."`,
        );
      }
    });
    ...
    if (userInteractionCount === 0) {
      ...
      logger.info(
        `No user messages found in logs, defaulting to 1 interaction for task ${this.segment.taskNumber}`,
      );
    }
  ...
  async calculateTokenMetrics(): Promise<TokenMetrics> {
    ...
    logger.info(
      `Starting token calculation for task ${this.segment.taskNumber}`,
    );
      ...
      if (message.type === 'say' && message.text) {
        try {
          ...
          if (data.tokensIn !== undefined) {
            ...
            messagesWithTokens++;
            logger.info(
              `Message ${messagesWithTokens} tokens - In: ${data.tokensIn}, Out: ${data.tokensOut ?? 0}`,
            );
          }
        } catch (e) {
          ...
          const cacheWritesMatch = message.text.match(
            /cacheWrites["\s:]+(\d+)/i,
          );
          ...
          if (tokensInMatch || tokensOutMatch) {
            ...
            messagesWithTokens++;
            logger.info(
              `Message ${messagesWithTokens} tokens - In: ${tokensInMatch?.[1] ?? 0}, Out: ${tokensOutMatch?.[1] ?? 0}`,
            );
          }
        }
      }
    ...
    if (this.segment.apiCalls?.length) {
      ...
      this.segment.apiCalls.forEach((apiCall: ApiCall) => {
        if (apiCall.usage) {
          ...
          apiCallsWithTokens++;
          logger.info(
            `API Call ${apiCallsWithTokens} tokens - In: ${apiCall.usage.input_tokens ?? 0}, Out: ${apiCall.usage.output_tokens ?? 0}`,
          );
        }
      });
    }
    ...
    return {
      ...
      cacheReads,
      conversationHistoryIndex:
        highestConvHistoryIndex > -1 ? highestConvHistoryIndex : 0,
    };
  }
   ...
   */
  async determineModel(): Promise<string> {
    ...
    if (modelFromLogs) {
      ...
      if (modelName) {
        logger.info(`Detected model from logs: ${modelName}`);
        return modelName;
      }
    }
