...
// Constants
const CLAUDE_LOGS_DIR =
  "/Users/ktalebian/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/tasks";
 ...
 */
async function extractChatMetrics(): Promise<ExtractionResult> {
  // Ensure metrics directory exists
  try {
    await fs.access(METRICS_DIR);
  } catch (error) {
    logger.error(
      `Metrics directory ${METRICS_DIR} does not exist. Please create it before running this script.`,
    );
    return ExtractionResult.error(
      `Metrics directory ${METRICS_DIR} does not exist`,
    );
  }
  ...
  try {
    ...
    const processingResults = await Promise.all(
      chatDirs.map((dir) =>
        processDirectory(dir, MODEL_ARG, CLIENT_ARG, SERVER_ARG),
      ),
    );
 ...
 */
async function getChatDirectories(): Promise<DirectoryResult> {
  ...
  try {
    ...
    return { success: false, directories: [], error: (error as Error).message };
  }
}
