...
async function loadData() {
    try {
        ...
        const mcpServers = [
            ...new Set(data.map((s) => s.mcpServer ?? "Unknown")),
        ];
        ...
        const mcpClients = [
            ...new Set(data.map((s) => s.mcpClient ?? "Unknown")),
        ];
        ...
        const models = [
            ...new Set(data.map((s) => s.model ?? "Unknown")),
        ];
...
function filterData() {
    ...
    filteredSessions = allSessions.filter((session) => {
        const matchesMcpServer = mcpServerFilter === "all" ||
            (session.mcpServer || "Unknown") === mcpServerFilter;
        const matchesMcpClient = mcpClientFilter === "all" ||
            (session.mcpClient || "Unknown") === mcpClientFilter;
        const matchesModel = modelFilter === "all" ||
            session.model === modelFilter ||
            (!session.model && modelFilter === "unknown");
...
function displayMetrics() {
    ...
    const performanceElements = {
        improvementTime: -parseFloat(improvements.time === "N/A" ? "0" : improvements.time),
        improvementCalls: -parseFloat(improvements.calls === "N/A" ? "0" : improvements.calls),
        improvementInteractions: -parseFloat(improvements.interactions === "N/A" ? "0" : improvements.interactions),
        improvementTokens: -parseFloat(improvements.tokens === "N/A" ? "0" : improvements.tokens),
        improvementCacheReads: -parseFloat(improvements.cacheReads === "N/A" ? "0" : improvements.cacheReads),
        improvementCacheWrites: -parseFloat(improvements.cacheWrites === "N/A" ? "0" : improvements.cacheWrites),
        improvementCost: -parseFloat(improvements.cost === "N/A" ? "0" : improvements.cost),
        improvementSuccess: parseFloat(improvements.success === "N/A" ? "0" : improvements.success),
    };
    ...
    const executiveSummaryUpdates = {
        apiCallsReduction: (-parseFloat(improvements.calls === "N/A" ? "0" : improvements.calls)).toFixed(2),
        interactionsReduction: (-parseFloat(improvements.interactions === "N/A" ? "0" : improvements.interactions)).toFixed(2),
        successImprovement: parseFloat(improvements.success === "N/A" ? "0" : improvements.success).toFixed(2),
        tokenIncrease: parseFloat(improvements.tokens === "N/A" ? "0" : improvements.tokens).toFixed(2),
        costIncrease: parseFloat(improvements.cost === "N/A" ? "0" : improvements.cost).toFixed(2),
        cacheWritesIncrease: parseFloat(improvements.cacheWrites === "N/A" ? "0" : improvements.cacheWrites).toFixed(2),
    };
    ...
    if (summaryEl) {
        summaryEl.innerHTML = Object.entries(metrics)
            ...
            const changeClass = (title === "Success Rate"
                ? parseFloat(change === "N/A" ? "0" : change) > 0
                : parseFloat(change === "N/A" ? "0" : change) < 0)
                ? "change-positive"
                : "change-negative";
            return `
...
function displayModelMetrics() {
    ...
    for (const [model, data] of Object.entries(models)) {
        ...
        if (data.control.length === 0 || data.mcp.length === 0)
            continue;
        ...
        data.control.forEach((session) => {
            taskCounts.control[session.taskId] =
                (taskCounts.control[session.taskId] || 0) + 1;
        });
        // Count MCP runs per task
        data.mcp.forEach((session) => {
            taskCounts.mcp[session.taskId] =
                (taskCounts.mcp[session.taskId] || 0) + 1;
        });
        const metrics = {
            Duration: {
    ...
    if (modelMetricsEl.children.length === 0) {
        ...
        messageEl.innerHTML =
            "<p>No model-specific data available for the selected filters.</p>";
...
function updateCharts() {
    ...
    const activeMetric = activeTab
        ? activeTab.getAttribute("data-chart") || "duration"
        : "duration";
    ...
    switch (metric) {
        case "duration":
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const controlDurations = taskData[taskId].control
                    .map((s) => s.duration)
                    .filter((d) => d !== null && d !== undefined);
                const mcpDurations = taskData[taskId].mcp
                    .map((s) => s.duration)
                    .filter((d) => d !== null && d !== undefined);
        ...
        case "apiCalls":
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const controlCalls = taskData[taskId].control
                    .map((s) => s.apiCalls)
                    .filter((c) => c !== null && c !== undefined);
                const mcpCalls = taskData[taskId].mcp
                    .map((s) => s.apiCalls)
                    .filter((c) => c !== null && c !== undefined);
        ...
        case "interactions":
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const controlInteractions = taskData[taskId].control
                    .map((s) => s.interactions)
                    .filter((i) => i !== null && i !== undefined);
                const mcpInteractions = taskData[taskId].mcp
                    .map((s) => s.interactions)
                    .filter((i) => i !== null && i !== undefined);
        ...
        case "tokens":
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const controlTokens = taskData[taskId].control
                    .map((s) => s.totalTokens || 0)
                    .filter((t) => t !== null && t !== undefined);
                const mcpTokens = taskData[taskId].mcp
                    .map((s) => s.totalTokens || 0)
                    .filter((t) => t !== null && t !== undefined);
        ...
        case "cacheReads":
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const controlReads = taskData[taskId].control
                    .map((s) => s.cacheReads || 0)
                    .filter((r) => r !== null && r !== undefined);
                const mcpReads = taskData[taskId].mcp
                    .map((s) => s.cacheReads || 0)
                    .filter((r) => r !== null && r !== undefined);
        ...
        case "cacheWrites":
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const controlWrites = taskData[taskId].control
                    .map((s) => s.cacheWrites || 0)
                    .filter((w) => w !== null && w !== undefined);
                const mcpWrites = taskData[taskId].mcp
                    .map((s) => s.cacheWrites || 0)
                    .filter((w) => w !== null && w !== undefined);
        ...
        case "cost":
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                const controlCost = taskData[taskId].control
                    .map((s) => s.cost || 0)
                    .filter((c) => c !== null && c !== undefined);
                const mcpCost = taskData[taskId].mcp
                    .map((s) => s.cost || 0)
                    .filter((c) => c !== null && c !== undefined);
...
function downloadCsv() {
    ...
    // Add headers
    csvContent +=
        "Directory ID,MCP Server,MCP Client,Task ID,Mode,Model,Duration (s),API Calls (count),User Interactions (count),Tokens (count),Cache Reads (count),Cache Writes (count),Cost ($),Success,Notes\n";
    // Add data rows
...
function average(values) {
    ...
    return validValues.length
        ? validValues.reduce((sum, val) => sum + (val || 0), 0) / validValues.length
        : 0;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (taskTabs) {
        taskTabs.addEventListener("click", (e) => {
            ...
            if (target.classList.contains("tab")) {
                ...
                const taskContent = document.getElementById(`task-${taskId}`);
                if (taskContent) {
    ...
    const chartTabs = document.querySelector(".tabs:not(#taskTabs)");
    if (chartTabs) {
        chartTabs.addEventListener("click", (e) => {
            const target = e.target;
            if (target.classList.contains("tab")) {
                // Remove active class from all tabs
                target.parentElement?.querySelectorAll(".tab").forEach((tab) => {
                    tab.classList.remove("active");
                });
                // Add active class to clicked tab
                target.classList.add("active");
                // Update chart with selected metric
                const metric = target.getAttribute("data-chart");
                if (metric) {
    ...
    const mcpServerFilter = document.getElementById("mcpServerFilter");
    if (mcpServerFilter) {
        mcpServerFilter.addEventListener("change", () => {
            filterData();
            displayMetrics();
            displayModelMetrics();
            displaySessions();
    ...
    const mcpClientFilter = document.getElementById("mcpClientFilter");
    if (mcpClientFilter) {
        mcpClientFilter.addEventListener("change", () => {
            filterData();
            displayMetrics();
            displayModelMetrics();
            displaySessions();
