...
class MetricsCalculator {
    ...
    async calculate() {
        // Validate input
        if (!this.segment) {
            logger_1.default.error('Cannot calculate metrics: segment is missing');
        ...
        try {
            ...
            const [apiCallCount, userMessageCount, tokenMetrics, model] = await Promise.all([
                this.calculateApiCalls(),
                this.calculateUserMessages(),
                this.calculateTokenMetrics(),
                this.determineModel()
            ]);
            const { tokensIn = 0, tokensOut = 0, totalCost = 0, cacheWrites = 0, cacheReads = 0, conversationHistoryIndex = 0 } = tokenMetrics ?? DEFAULT_METRICS;
            ...
            if (duration < 0 || duration > MAX_DURATION) {
                logger_1.default.warn(`Invalid duration detected for task ${this.segment.taskNumber}: ${duration}ms. Capping at 24 hours.`);
            ...
            logger_1.default.error(`Error calculating metrics: ${error.message}`);
    ...
    checkForMcpUsage() {
        ...
        return this.segment.apiCalls.some(apiCall => {
            if (apiCall.role === 'assistant' && apiCall.content && Array.isArray(apiCall.content)) {
                return apiCall.content.some(content => content.type === 'text' && content.text &&
    ...
    async calculateApiCalls() {
        ...
        this.segment.userMessages.forEach((msg) => {
            ...
            if (msg.type === 'say' && msg.say === 'api_req_started') {
                apiCallCount++;
                logger_1.default.debug(`Found API request in task ${this.segment.taskNumber}: ${msg.text ?? '[no text]'}`);
            }
        });
        logger_1.default.info(`API Call Count for Task ${this.segment.taskNumber}: ${apiCallCount}`);
        return apiCallCount;
    }
    /**
    ...
    async calculateUserMessages() {
        ...
        if (!this.segment?.apiCalls?.length) {
            logger_1.default.info(`No API calls found, defaulting to 1 interaction for task ${this.segment?.taskNumber ?? 'unknown'}`);
            return 1;
        }
        ...
        this.segment.apiCalls.forEach(entry => {
            ...
            if (!foundInitialTask && (fullText.includes('Complete Task') ||
                ...
                logger_1.default.info(`Found initial task instruction for task ${this.segment.taskNumber}`);
            ...
            if (foundInitialTask &&
                ...
                logger_1.default.info(`Found follow-up user message for task ${this.segment.taskNumber}: "${fullText.substring(0, 50)}..."`);
            }
        });
        // If we still haven't found any interactions, default to 1
        if (userInteractionCount === 0) {
            userInteractionCount = 1;
            logger_1.default.info(`No user messages found in logs, defaulting to 1 interaction for task ${this.segment.taskNumber}`);
        }
    ...
    async calculateTokenMetrics() {
        ...
        logger_1.default.info(`Starting token calculation for task ${this.segment.taskNumber}`);
            ...
            if (message.type === 'say' && message.text) {
                try {
                    ...
                    if (data.tokensIn !== undefined) {
                        tokensIn += parseInt(data.tokensIn.toString(), 10);
                        tokensOut += parseInt((data.tokensOut ?? 0).toString(), 10);
                        totalCost += parseFloat((data.cost ?? 0).toString());
                        // Track cache metrics if available
                        if (data.cacheWrites !== undefined) {
                            cacheWrites += parseInt(data.cacheWrites.toString(), 10);
                        }
                        if (data.cacheReads !== undefined) {
                            cacheReads += parseInt(data.cacheReads.toString(), 10);
                        }
                        messagesWithTokens++;
                        logger_1.default.info(`Message ${messagesWithTokens} tokens - In: ${data.tokensIn}, Out: ${data.tokensOut ?? 0}`);
                    }
                }
                catch (e) {
                    ...
                    if (tokensInMatch || tokensOutMatch) {
                        ...
                        if (cacheWritesMatch?.[1])
                            cacheWrites += parseInt(cacheWritesMatch[1], 10);
                        if (cacheReadsMatch?.[1])
                            cacheReads += parseInt(cacheReadsMatch[1], 10);
                        messagesWithTokens++;
                        logger_1.default.info(`Message ${messagesWithTokens} tokens - In: ${tokensInMatch?.[1] ?? 0}, Out: ${tokensOutMatch?.[1] ?? 0}`);
                    }
                }
            }
            // Track conversation history index
            if (message.conversationHistoryIndex !== undefined) {
                const indexValue = parseInt(message.conversationHistoryIndex.toString(), 10);
        ...
        if (this.segment.apiCalls?.length) {
            ...
            this.segment.apiCalls.forEach((apiCall) => {
                if (apiCall.usage) {
                    if (apiCall.usage.input_tokens)
                        tokensIn += apiCall.usage.input_tokens;
                    if (apiCall.usage.output_tokens)
                        tokensOut += apiCall.usage.output_tokens;
                    if (apiCall.usage.cost)
                        totalCost += apiCall.usage.cost;
                    apiCallsWithTokens++;
                    logger_1.default.info(`API Call ${apiCallsWithTokens} tokens - In: ${apiCall.usage.input_tokens ?? 0}, Out: ${apiCall.usage.output_tokens ?? 0}`);
                }
            });
        }
        ...
        logger_1.default.info(`Token metrics for task ${this.segment.taskNumber}:
    ...
    async determineModel() {
        ...
        if (this.modelArg) {
            logger_1.default.info(`Using model from command-line argument: ${this.modelArg}`);
            return this.modelArg;
        }
        // Check if we have API calls to analyze
        if (!this.segment?.apiCalls?.length) {
            const defaultModel = 'claude-3.7-sonnet';
            logger_1.default.info(`No API calls found, defaulting to: ${defaultModel}`);
            return defaultModel;
        }
        ...
        if (modelFromLogs) {
            ...
            modelFromLogs.content?.forEach(content => {
                if (content.type === 'text' && content.text) {
                    const match = content.text.match(/You are a powerful agentic AI coding assistant, powered by (Claude [\d.]+ \w+)/i);
                    if (match?.[1]) {
            ...
            if (modelName) {
                logger_1.default.info(`Detected model from logs: ${modelName}`);
                return modelName;
            }
        }
        // Default if not found in logs and no argument provided
        const defaultModel = 'claude-3.7-sonnet';
        logger_1.default.info(`Model not found in logs or args, defaulting to: ${defaultModel}`);
        return defaultModel;
    }
}
