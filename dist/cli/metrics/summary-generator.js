...
class SummaryGenerator {
    ...
    async generateSummary(taskMetrics) {
        ...
        try {
            ...
            const summaryPath = path_1.default.join(this.metricsDir, "summary.json");
            await fs_1.promises.writeFile(summaryPath, JSON.stringify(uniqueMetrics, null, 2));
            ...
            logger_1.default.error("Error generating summary:", error);
    ...
    async writeIndividualMetricFiles(taskMetrics) {
        ...
        for (const metric of taskMetrics) {
            try {
                ...
                const tasksDir = path_1.default.join(this.metricsDir, "tasks");
                // Ensure tasks directory exists
                try {
                    await fs_1.promises.mkdir(tasksDir, { recursive: true });
                }
                catch (mkdirError) {
                    logger_1.default.warn(`Could not create tasks directory: ${mkdirError.message}`);
                }
                const filePath = path_1.default.join(tasksDir, filename);
                // Log the metric before writing to help debug
                logger_1.default.info(`Writing metric file for task ${metric.taskId} with ${metric.apiCalls ?? 0} API calls`);
                ...
                logger_1.default.error(`Error writing metric file for task ${metric.taskId}: ${error.message}`);
    ...
    async generateSummaryFromFiles() {
        logger_1.default.info("Generating summary from individual metric files...");
        // Look for metric files in the 'tasks' subdirectory
        const tasksDir = path_1.default.join(this.metricsDir, "tasks");
        let files;
        try {
            files = await fs_1.promises.readdir(tasksDir);
        }
        catch (error) {
            logger_1.default.error(`Failed to read tasks directory: ${error.message}`);
        ...
        if (metricFiles.length === 0) {
            logger_1.default.warn("No metric files found");
            return SummaryResult.error("No metric files found", []);
        }
        logger_1.default.info(`Found ${metricFiles.length} individual metric files`);
        // Read and parse each file
        const allMetrics = [];
        const failedFiles = [];
        for (const file of metricFiles) {
            const filePath = path_1.default.join(this.metricsDir, "tasks", file);
            try {
                ...
                logger_1.default.error(`Error parsing metric file ${file}: ${error.message}`);
        ...
        try {
            // Write the summary file
            const summaryPath = path_1.default.join(this.metricsDir, "summary.json");
            await fs_1.promises.writeFile(summaryPath, JSON.stringify(allMetrics, null, 2));
            ...
            logger_1.default.error(`Failed to write summary file: ${error.message}`);
    ...
    async mergeWithExistingSummary(newMetrics) {
        ...
        try {
            ...
            logger_1.default.warn(`Could not read existing summary: ${error.message}. Creating new summary.`);
        }
        ...
        for (const newMetric of newMetrics) {
            ...
            if (duration < 0 || duration > MAX_DURATION) {
                logger_1.default.warn(`Invalid duration detected in metric for task ${newMetric.taskId}: ${duration}ms. Adjusting...`);
        ...
        try {
            ...
            logger_1.default.error(`Error writing merged summary: ${error.message}`);
    ...
    async metricFileExists(mode, taskId, directoryId) {
        ...
        try {
            const tasksDir = path_1.default.join(this.metricsDir, "tasks");
            const files = await fs_1.promises.readdir(tasksDir);
            if (directoryId) {
            ...
            logger_1.default.error(`Error checking for existing metric file: ${error.message}`);
            return false;
        }
    }
    /**
    ...
    printSummaryStatistics(taskMetrics) {
        if (!taskMetrics?.length) {
            logger_1.default.info("No metrics to display");
            return;
        }
        ...
        logger_1.default.info("-------------------------");
        logger_1.default.info(`Total tasks processed: ${taskMetrics.length}`);
        logger_1.default.info(`Control tasks: ${controlTasks.length}`);
        logger_1.default.info(`MCP tasks: ${mcpTasks.length}`);
        logger_1.default.info("\nTask Details:");
        taskMetrics.forEach((task) => {
            logger_1.default.info(`Task ${task.taskId ?? "unknown"} (${task.mode ?? "unknown"}): duration=${((task.duration ?? 0) / 1000).toFixed(1)}s, interactions=${task.interactions ?? 0}, apiCalls=${task.apiCalls ?? 0}, tokens=${task.tokensIn ?? 0}`);
        });
        if (controlTasks.length > 0 && mcpTasks.length > 0) {
            this.printPerformanceComparison(controlTasks, mcpTasks);
        }
        logger_1.default.info(`Summary file generated at: ${path_1.default.join(this.metricsDir, "summary.json")}`);
    }
...
exports.default = SummaryGenerator;
