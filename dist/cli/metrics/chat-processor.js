...
class ChatProcessor {
    constructor(chatDir) {
        ...
        this.apiHistory = [];
        this.uiMessages = [];
        this.directoryId = path_1.default.basename(chatDir);
    ...
    async process() {
        try {
            ...
            if (!initialized) {
                logger_1.default.error(`Failed to initialize chat processor for ${this.directoryId}`);
                return [];
            }
            // Identify test type with validation
            const testType = this.identifyTestType();
            if (!testType) {
                logger_1.default.warn(`Could not identify test type for ${this.directoryId}, skipping processing`);
                return [];
            }
            ...
            logger_1.default.error(`Error processing chat ${this.directoryId}: ${error.message}`);
            return [];
        }
    }
    /**
    ...
    async initialize() {
        ...
        try {
            const [apiHistoryContent, uiMessagesContent] = await Promise.all([
                fs_1.promises.readFile(path_1.default.join(this.chatDir, "api_conversation_history.json"), "utf8"),
                fs_1.promises.readFile(path_1.default.join(this.chatDir, "ui_messages.json"), "utf8"),
            ]);
            ...
            logger_1.default.error(`Error initializing chat processor: ${error.message}`);
            return false;
        }
    }
    /**
    ...
    identifyTestType() {
        ...
        for (const message of this.uiMessages) {
            if (message.type === "say" && message.say === "text" && message.text) {
                if (message.text.includes("'agent-instructions/control_instructions.md'") ||
                    ...
                    message.text.includes(types_1.CONTROL_MARKER)) {
                    return "control";
                }
                if (message.text.includes("'agent-instructions/mcp_instructions.md'") ||
                    ...
                    message.text.includes(types_1.MCP_MARKER)) {
                    return "mcp";
                }
            }
            // Check for MCP usage
        ...
        for (const entry of this.apiHistory) {
            if (entry.role === "user" &&
                ...
                if (content.includes(types_1.CONTROL_MARKER) ||
                    content.includes("'agent-instructions/control_instructions.md'") ||
                    content.includes('"agent-instructions/control_instructions.md"')) {
                    return "control";
                }
                if (content.includes(types_1.MCP_MARKER) ||
    ...
    extractTaskSegments(testType) {
        if (!this.initialized || !this.uiMessages?.length) {
            logger_1.default.warn("Cannot extract task segments: chat data not initialized");
        ...
        if (taskBoundaries.length === 0) {
            logger_1.default.warn(`No task boundaries found in ${this.directoryId}`);
            return Promise.resolve([]);
        }
        ...
        for (let i = 0; i < taskBoundaries.length; i++) {
            ...
            if (!taskBoundaries[i].endTime ||
                ...
                logger_1.default.warn(`Invalid endTime detected for task ${taskBoundaries[i].taskNumber}. Using last valid message timestamp.`);
                taskBoundaries[i].endTime = lastValidMessageTs;
            }
        }
    ...
    async processTaskSegment(boundary) {
        ...
        const messages = this.uiMessages.slice(boundary.startIndex, (boundary.endIndex ?? this.uiMessages.length - 1) + 1);
        logger_1.default.info(`Processing ${messages.length} messages for task ${boundary.taskNumber}`);
